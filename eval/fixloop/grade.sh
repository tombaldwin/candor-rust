...
POLICY="$HOME/git/candor-rust/eval/fixloop/fixture-orderflow/.candor-policy"   # the CANONICAL policy (agents can't relax it)
W=$(mktemp -d)
...
NET=$("$QUERY" where "$W/rep" Net 0 2>/dev/null | grep -cE "::")
