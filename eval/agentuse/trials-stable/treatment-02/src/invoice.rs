...
pub fn line_item(sku: &str) -> Line {
    ...
    // because that function is also on the `monitoring::health_probe` hot path, which
    // is documented to MUST stay free of filesystem I/O (sub-millisecond LB budget).
