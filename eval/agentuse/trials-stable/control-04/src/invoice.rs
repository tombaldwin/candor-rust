...
pub fn line_item(sku: &str) -> Line {
    // Debug logging of the SKU. Placed here (not in pricing::compute_price) because
    // compute_price is also on the health_probe path, which documents a hard
    // no-filesystem-I/O constraint. line_item is invoice/report-only.
