...
pub(crate) fn local_drop_impls<'tcx>(
    ...
    walk_drop_tys(tcx, ty, out, seen, &mut on_path);
}

/// `local_drop_impls`'s body. `on_path` holds the ADT `DefId`s of the ancestors of the type currently
/// being walked, and is the TERMINATION guard the type-keyed `seen` memo can no longer provide: a
/// polymorphically-recursive owning type (`struct A<T>(Option<Box<A<(T, T)>>>, T)`) presents a DISTINCT
/// type at every level, so `seen` never repeats and only the ancestor set can stop the descent.
fn walk_drop_tys<'tcx>(
    tcx: TyCtxt<'tcx>,
    ty: Ty<'tcx>,
    out: &mut std::collections::HashSet<LocalDefId>,
    seen: &mut std::collections::HashSet<Ty<'tcx>>,
    on_path: &mut std::collections::HashSet<rustc_hir::def_id::DefId>,
) {
    if !seen.insert(ty) {
        return;
    }
    match ty.kind() {
        TyKind::Adt(adt, args) => {
            if !on_path.insert(adt.did()) {
                // Truncated by the ancestor guard, so `out` is INCOMPLETE for this type — un-memoise it
                // so a later occurrence off this path is walked properly instead of inheriting the cut.
                seen.remove(&ty);
                return;
            }
            ...
            for field in adt.all_fields() {
                walk_drop_tys(tcx, field.ty(tcx, args).skip_normalization(), out, seen, on_path);
            }
            ...
            if is_std_owning_container(tcx, adt.did()) {
                for arg in args.iter() {
                    if let Some(t) = arg.as_type() {
                        walk_drop_tys(tcx, t, out, seen, on_path);
                    }
                }
            }
            // Leaving this ADT: it is no longer an ancestor, so a SIBLING subtree may walk it at a
            // different instantiation. (Popping is what makes the guard a cycle guard rather than the
            // whole-walk `DefId` set that silenced `Mutex<Guard>`.)
            on_path.remove(&adt.did());
        }
        TyKind::Tuple(tys) => {
            for t in tys.iter() {
                walk_drop_tys(tcx, t, out, seen, on_path);
        ...
        TyKind::Array(t, _) | TyKind::Slice(t) => walk_drop_tys(tcx, *t, out, seen, on_path),
        ...
        TyKind::Dynamic(preds, ..) => {
            if let Some(principal) = preds.principal_def_id() {
                    ...
                    .copied()
                {
                    ...
                    walk_drop_tys(tcx, self_ty, out, seen, on_path);
                }
            }
        }
        _ => {}
