...
/// when `Self` is generic. A non-trait `DefId` already names its own body and is returned unchanged.
fn fn_value_targets<'tcx>(
    ...
    // exists to prevent.
    let cha = cha_targets(cx.tcx, did);
    if cha.is_empty() { vec![did] } else { cha }
}
