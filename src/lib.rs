...
///
/// PADDING IS NOT UNIFORM, and getting that wrong is a SILENT UNDER-REPORT, not a crash. The two halves
/// of an assoc fn's arg list answer different questions:
///
/// * the PARENT (trait) parameters CHOOSE THE IMPL. Filling one with a placeholder does not degrade
///   precision, it asks about a different impl — and the honest answer to "which impl is
///   `<Eq00 as PartialEq<()>>::eq`?" is `None`. MEASURED (soundness seeds 7/17/26/27/31/33/46/48/50/56,
///   `vec_contains_eq`): `PartialEq<Rhs = Self>` reached here as `known = [Eq00]`, was padded to
///   `[Eq00, ()]`, resolved to nothing, and the driver edge to a LOCAL effectful `eq` vanished — ten
///   fuzzer seeds went `pure/omitted` over a real effect. So an unknown trait parameter is filled from
///   the parameter's OWN DECLARED DEFAULT (`Rhs = Self`, instantiated against the args already built —
///   the language's answer, not a guess of ours), and when it has no default this REFUSES.
/// * the METHOD'S OWN parameters ride along without selecting anything, so `()` / erased regions are
///   genuinely inert there. That is the half `Hash::hash<H: Hasher>` needed, and the whole reason this
///   function exists.
///
/// A CONST parameter gets no filler either way, so this REFUSES rather than invent one — no std trait
/// these call sites name has one, and refusing is the fail-closed direction if that ever changes.
///
/// Refusing loses an edge, which is the same direction as the bug above; it is chosen only where the
/// alternative is inventing an impl selector. Every trait these call sites currently name
/// (`Deref`/`From`/`FromStr`/`FromIterator`/`PartialEq`/`PartialOrd`/`Ord`/`Clone`/`Hash`/`Display`/
/// `Iterator`/`IntoIterator`/`fmt::Write`/`io::Write`/`io::Read`) either has its trait parameters fully
/// named by its call site or defaults them to `Self`. MEASURED, not assumed: with the refusal branch
/// traced, it fired ZERO times over the 60 default fuzzer seeds, all 21 `soundness/realworld` driver
/// crates (with their real third-party dependency trees) and `sample/`. If it ever does fire it drops an
/// edge silently, so that is the number to re-measure before adding a trait to the driver tables above.
fn trait_args_for<'tcx>(
    ...
    // Params with `index < parent_count` belong to the TRAIT and select the impl; the rest are the
    // method's own. `for_item` cannot bail, so decide the refusals in a pre-pass over the same params.
    let parent_count = cx.tcx.generics_of(def_id).parent_count;
    let mut generics = Some(cx.tcx.generics_of(def_id));
    while let Some(g) = generics {
        if g.own_params.iter().any(|p| {
            known.get(p.index as usize).is_none()
                && match p.kind {
                    ParamKind::Lifetime => false,
                    ParamKind::Const { .. } => true,
                    ParamKind::Type { has_default, .. } => {
                        (p.index as usize) < parent_count && !has_default
                    }
                }
        }) {
            return None;
    ...
    Some(rustc_middle::ty::GenericArgs::for_item(cx.tcx, def_id, |param, built| match param.kind {
        ParamKind::Lifetime => cx.tcx.lifetimes.re_erased.into(),
        _ => match known.get(param.index as usize) {
            Some(t) => (*t).into(),
            // A defaulted parameter's default may mention earlier ones (`Rhs = Self`), so instantiate it
            // against the args built so far — `built` is exactly that prefix.
            None if matches!(param.kind, ParamKind::Type { has_default: true, .. }) => {
                cx.tcx.type_of(param.def_id).instantiate(cx.tcx, built).skip_normalization().into()
            }
