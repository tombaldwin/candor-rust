...
//
// Guard-deletion, both halves, each independently load-bearing: restore the `DefId` key and the three
// `decl_order_*` functions plus all three `via_{mutex,rwlock,refcell}` go silent; drop `PhantomData` from
// `is_std_owning_container` and `via_vec_into_iter` + `via_phantom_arena` go silent — while the other
// half of the file stays green either way.
#![allow(unused)]
