...
pub const SPEC_VERSION: &str = "0.35";
...
pub fn same_artifact(a: &str, b: &str) -> bool {
    ...
    if (ra != std::path::Path::new(a) || rb != std::path::Path::new(b))
        && let (Some(x), Some(y)) = (resolve(&ra.to_string_lossy()), resolve(&rb.to_string_lossy()))
        && x == y
    {
        return true;
    }
...
mod tests {
    ...
    fn round_trips() {
        ...
        assert!(s.contains("\"spec\":\"0.35\""), "envelope must carry the spec version: {s}");
