...
fn write_containment_report(dir: &std::path::Path, name: &str, web_has_db: bool) -> String {
    ...
    let report = format!(
        r#"{{"candor":{{"version":"scan-test","toolchain":"stable","spec": "0.20"}},"package":"cnt","functions":[
...
fn diff_reports_a_gained_effect() {
    ...
    let mk = |prefix: &str, effs: &str| {
        std::fs::write(format!("{prefix}.d.scan.json"), format!(
            r#"{{"candor":{{"version":"scan-test","toolchain":"stable","spec": "0.20"}},"functions":[
...
fn write_orderflow_fixture(f: &Fixture) {
    let report = r#"{
  "candor": { "version": "scan-test", "toolchain": "stable", "spec": "0.20" },
  "package": "of",
  "functions": [
...
fn gains_strict_exits_1_and_rejects_silently_swallowed_policy() {
    ...
    let base_report = r#"{"candor":{"version":"t","spec":"0.20"},"package":"lib","functions":[{"fn":"lib::f","loc":"s:1","inferred":["Fs"],"hash":"h"}]}"#;
    let cur_report = r#"{"candor":{"version":"t","spec":"0.20"},"package":"lib","functions":[{"fn":"lib::f","loc":"s:1","inferred":["Fs","Net"],"hash":"h"}]}"#;
