...
"));
    ...
    /// R109 (CARDINAL SIN, introduced by R106's own fix) — A NESTED BLOCK'S ITEM MUST NOT SHADOW THE
    /// WHOLE FUNCTION.
    ///
    ...
    fn r109_a_nested_block_item_does_not_shadow_the_whole_body() {
        ...
        for (tag, snippet) in [
            ...
            assert_eq!(
                ...
                "R109 ({tag}): the `struct Cmd` is declared in a SEPARATE scope, so the body's own \
    ...
    /// R109 — THE SIGNATURE IS NOT IN THE BODY'S SCOPE. A second position the flattened shadow reached,
    ...
    fn r109_a_body_item_does_not_shadow_the_signature() {
        ...
        assert_eq!(
            ...
            "R109: `c: &mut Cmd` names the file's import — a body-local `struct Cmd` cannot rebind a \
             parameter's type, and `c.status()` provably runs a process:\n{v:#}"
        );
        assert_eq!(
            effs_opt(&v, "sig_ctl"), vec!["Exec".to_string()],
            "R109 CONTROL: the same function with the body item RENAMED — the two arms differ in nothing \
             else, so a difference between them is the shadow and nothing else:\n{v:#}"
        );
    }

    /// R109 CONTROL — THE R106 PRECISION GAIN MUST SURVIVE THE SCOPING. Where a nested block declares a
    ...
    fn r109_a_nested_item_used_only_in_its_own_block_still_shadows() {
        ...
        assert!(
            ...
            "R109 CONTROL: every mention of `Cmd` in this body is inside the block that declares it, so \
             the shadow is exact — `Exec` here is R106's fabrication on a path that runs nothing:\n{v:#}"
        );
        assert_eq!(
            effs_opt(&v, "two_blocks"), vec!["Exec".to_string()],
            "R109 RESIDUAL: two sibling blocks declare the name, so no function-wide promotion is sound \
