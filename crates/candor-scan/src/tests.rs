...
//! kept verbatim: several tests embed column-sensitive source strings.
    ...
    fn dep_report_package_field_registers_coverage_for_the_ledger_exemption() {
        ...
        std::fs::write(d.join("purity-claim.json"), format!(r#"{{
            "candor": {{"version": "{me}", "toolchain": "stable", "spec": "0.12"}},
            "package": "dep-c",
            "functions": []}}"#)).unwrap();
...
";
    ...
    fn repo_docs_carry_the_family_attribution_and_spec_floor() {
        ...
        // family's deep Rust engine, spec floor 0.12. A cheap grep gate so a doc rewrite can't quietly
        // reintroduce "the reference implementation" or drop the spec-0.12 floor string. Skips outside
        ...
        assert!(readme.contains("spec 0.12"), "README must state the spec 0.12 floor");
        assert!(agents.contains("spec 0.12"), "AGENTS must state the spec 0.12 floor");
    }
    ...
    fn baseline_guard_resolution_union_and_gain_logic() {
        ...
        let report = |ver: &str| format!(
            r#"{{"candor":{{"version":"{ver}","toolchain":"stable","spec":"0.12"}},
